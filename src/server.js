...
import { appendOrderToSheet, generateOrderId } from './services/sheets.js';
import { createPixCharge, parsePaymentWebhook, fetchMercadoPagoPayment } from './services/billing.js';
import { issueInvoice } from './services/invoice.js';
import { savePendingPayment, getPendingPayment, clearPendingPayment, purgeExpiredPendingPayments } from './services/order-store.js';
...
async function finalizeOrder(request, reply) {
...
async function handlePaymentWebhook(request, reply) {
  ...
  const notification = await resolvePaymentNotification(request.body);
  ...
  if (!notification.isPaid) {
    ...
    return reply.send({ ignored: true, status: notification.status });
  }
  ...
  if (!orderId) {
    ...
    return reply.send({ ignored: true, reason: 'missing_external_reference' });
  }
  ...
  if (!pending) {
    ...
    return reply.send({ ignored: true, reason: 'pending_payment_not_found', orderId });
  }
...
app.post('/order/finalize', finalizeOrder);
